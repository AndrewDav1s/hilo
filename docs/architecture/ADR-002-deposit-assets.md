...
Assuming a placeholder token allow-list of one element (e.g. `uhilo`), and a uToken existing (e.g. `u-uhilo`), an end-to-end test can be created in which one user account sends a `MsgDeposit` and a `MsgWithdraw` of the appropriate token types.
