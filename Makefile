...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=hilo \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=hilod \
...
docker-build:
	@docker build -t hilonetwork/hilod .

docker-localnet-build:
	@docker build -t hilonetwork/hilod-localnet --file localnet.Dockerfile .
...
localnet-start: build-linux localnet-stop
  # start a local Hilo network if a network configuration does not already exist
	@if ! [ -f $(BUILD_DIR)/node0/hilod/config/genesis.json ]; then \
    docker run --rm -v $(BUILD_DIR):/hilod:Z hilonetwork/hilod-localnet \
