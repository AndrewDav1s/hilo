...
func (s *KeeperTestSuite) SetupTest() {
	...
	hiloApp := s.GetHiloApp(s.chainA)

	queryHelper := baseapp.NewQueryServerTestHelper(s.chainA.GetContext(), hiloApp.InterfaceRegistry())
	types.RegisterQueryServer(queryHelper, hiloApp.TransferKeeper)
	s.queryClient = types.NewQueryClient(queryHelper)
}

func (s *KeeperTestSuite) GetHiloApp(c *ibctesting.TestChain) *app.HiloApp {
	hiloApp, ok := c.App.(*app.HiloApp)
	s.Require().True(ok)
...
func (s *KeeperTestSuite) TestGetTransferAccount() {
	...
	macc := s.GetHiloApp(s.chainA).TransferKeeper.GetTransferAccount(s.chainA.GetContext())
...
func (s *KeeperTestSuite) TestTrackMetadata() {
	...
	s.Run("OnRecvPacketA", func() {
		...
		err := s.GetHiloApp(s.chainA).TransferKeeper.OnRecvPacket(s.chainA.GetContext(), packet, data)
	...
	s.Run("OnRecvPacketB", func() {
		...
		err := s.GetHiloApp(s.chainB).TransferKeeper.OnRecvPacket(s.chainB.GetContext(), packet, data)
		s.Require().NoError(err)
	})
	...
	s.Run("SendTransfer", func() {
		...
		registerDenom := func() {
			...
			if !s.GetHiloApp(s.chainB).TransferKeeper.HasDenomTrace(s.chainB.GetContext(), traceHash) {
				s.GetHiloApp(s.chainB).TransferKeeper.SetDenomTrace(s.chainB.GetContext(), denomTrace)
			}
		}

		registerDenom()

		err = s.GetHiloApp(s.chainB).TransferKeeper.SendTransfer(
			s.chainB.GetContext(),
			packet.SourcePort,
	...
	_, ok := s.GetHiloApp(s.chainA).BankKeeper.GetDenomMetaData(s.chainA.GetContext(), "quark")
	s.Require().True(ok)

	_, ok = s.GetHiloApp(s.chainB).BankKeeper.GetDenomMetaData(s.chainB.GetContext(), "photon")
	s.Require().True(ok)
}
