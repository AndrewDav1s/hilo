...
func localnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "localnet",
		Short: "Initialize files for an Hilo local network",
		Long: `Create the necessary initialization files and directories needed to
bootstrap a configurable local Hilo testing network. A certain number of
...
Note, strict routability for addresses is turned off in the config file.
	...
	cmd.Flags().String(flagNodeDaemonHome, "hilod", "Home directory of the node's configuration")
...
func initLocalnet(
	...
	hiloCfg := srvconfig.DefaultConfig()
	hiloCfg.MinGasPrices = cmdCfg.GetString(server.FlagMinGasPrices)
	hiloCfg.API.Enable = true
	hiloCfg.Telemetry.Enabled = true
	hiloCfg.Telemetry.PrometheusRetentionTime = 60
	hiloCfg.Telemetry.EnableHostnameLabel = false
	hiloCfg.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), hiloCfg)
	}
