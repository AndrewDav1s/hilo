...
// file.
func (app *HiloApp) ExportAppStateAndValidators(
	forZeroHeight bool,
...
func (app *HiloApp) prepForZeroHeightGenesis(ctx sdk.Context, jailAllowedAddrs []string) {
	...
	app.StakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
		...
		if err != nil {
			panic(fmt.Sprintf("failed to withdraw validator commission: %s", err))
		}
	...
	app.StakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
		// Donate any un-withdrawn outstanding reward fraction tokens to the
		// community pool.
	...
	if _, err := app.StakingKeeper.ApplyAndReturnValidatorSetUpdates(ctx); err != nil {
		panic(fmt.Sprintf("failed to apply validator updates: %s", err))
	}
